...
<div class="m-6">
    ...
    <div class="container position-relative">
        <div class="download_btn">
            <a onclick="saveAsPDF()" class="btn btn-sm btn-white btn-icon-only ml-2 shadow-sm">
                <span class="btn-inner--icon"><i class="ti ti-download"></i></span>
            </a>
        </div>
        <div class="row invoice" id="printableArea">
            <div class="col-12 card p-4">
                <div class="invoice-details">
                    <h1 class="invoice-id h4">{{ $details['invoice_id'] }}</h1>
                    <div class="date mb-3">{{ __('Date of Invoice') }}: {{ $details['date'] }}</div>
                <div class="date mb-3 float-right">{!! DNS2D::getBarcodeHTML(route('purchase.link.copy',\Illuminate\Support\Facades\Crypt::encrypt($purchase->id)),'QRCODE',2,2) !!}</div>
                ...
                <div class="">
                    ...
                    @if ($details['pay'] == 'show')
                        <button class="btn btn-primary btn-sm btn-done-payment text-right float-right rounded-pill"
                            data-url="{{ route('purchases.store') }}">{{ __('Done Payment') }}</button>
                    @endif
                </div>
            </div>
        </div>
