    ...
    public static function getProductBrandAnalysis(array $data)
    {
        ...
        if ($brands->count() > 0) {
            ...
            foreach($brands->get() as $counter => $brand) {
                ...
                $profitlossdisplay = '<span class="badge p-2 px-3 rounded bg-' . (($sold_price - $purchased_price < 0) ? 'danger' : 'success') . '">' . number_format($sold_price - $purchased_price, 2) . '</span>';
        ...
        $data['totalProfitOrLoss'] = '<span class="badge p-2 px-3 rounded bg-' . (($total_profit_or_loss < 0) ? 'danger' : 'success') . '">' . number_format($total_profit_or_loss, 2) . '</span>';
